...
void mouseMoved() {
  ...
  for ( int i=0; i<cantidad; i++ ) {
    if (objeto[i].vivo == false) { 
      objeto[i].vivo = true;
      objeto[i].vida = 10; //50 cantidad de cuadros que permanece en pantalla
...
void addTuioCursor(TuioCursor tcur) {
  ...
  for ( int i=0; i<cantidad; i++ ) {
    if (objeto[i].vivo == false) { 
      objeto[i].vivo = true;
      objeto[i].vida = 10; //cantidad de cuaros que permanece en pantalla


      //------MAPEO DE DISTORCION DE CAMARA ------
      //  float faseX = map(tcur.getScreenX(width), 172, 529, 0, width);
      //float faseY = map(tcur.getScreenY(height), 230, 240, 0, height);

      int donde_inicia_x = 100;//donde inicia la posicion del robot
      int donde_inicia_y = 100;//donde inicia la posicion del robot
      int donde_termina_x = 600;//donde termina la posicion del robot
      int donde_termina_y = 600;//donde termina la posicion del robot

      int correxion_inicio_x = 5; // para correr hacia un costado
      int correxion_inicio_y = 5;

      float estiro_hacia_el_final_x = 1.3; //multiplicar por .0 para acortar y por 1.0 para aumentar
